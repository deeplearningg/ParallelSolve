...
double gWeightLevelDecreaingRate = 1.0;//1.0
...
bool handleArgument( const int& argc, char* argv[], CParamPlacement& param )
{
    ...
    param.sg_namdaIncFactor = 35;
    param.sg_stepsizeInc = 2.0;
    param.sg_Modify = 0.5;
    param.sg_stepsize = 0.9;
    ...
    while( i < argc )
    {
        ...
        //Added by flpeng 20160401
        else if( strcmp( argv[i]+1, "sg_namdaIncFactor" ) == 0 )
            param.sg_namdaIncFactor = atof( argv[++i] );
        else if( strcmp( argv[i]+1, "sg_stepsizeInc" ) == 0 )
            param.sg_stepsizeInc = atof( argv[++i] );
        else if( strcmp( argv[i]+1, "sg_Modify" ) == 0 )
            param.sg_Modify = atof( argv[++i] );
        else if( strcmp( argv[i]+1, "sg_stepsize" ) == 0 )
            param.sg_stepsize = atof( argv[++i] );
        else if( strcmp( argv[i]+1, "sg_clusterFactor" ) == 0 )
        {
            param.sg_clusterFactor = atoi( argv[++i] );
            gClusterRatio = param.sg_clusterFactor;
        }
        // added by hdgao
        else if( strcmp( argv[i]+1, "r_cycle" ) == 0 )
            param.run_cycle = atof( argv[++i] );
        else if( strcmp( argv[i]+1, "isparallel" ) == 0 ){
            double temp = atof( argv[++i] );
            if(temp == 0.0)
                param.isParallel = false;
        }
        else if( strcmp( argv[i]+1, "loop" ) == 0 ){
            param.loop = atof( argv[++i] );
        }
        ...
        else if( strcmp( argv[i]+1, "checkstep" ) == 0 ){
            param.checkstep = atof( argv[++i] );
        }
...
int main(int argc, char* argv[])
{
    ...
    std::chrono::high_resolution_clock::time_point GlobalPlacementStartTime_multithread;
    std::chrono::high_resolution_clock::time_point GlobalPlacementEndTime_multithread;

    std::chrono::high_resolution_clock::time_point DetailPlacementStartTime_multithread;
    std::chrono::high_resolution_clock::time_point DetailPlacementEndTime_multithread;

    std::chrono::high_resolution_clock::time_point LegalizationStartTime_multithread;
    std::chrono::high_resolution_clock::time_point LegalizationEndTime_multithread;

    std::chrono::high_resolution_clock::time_point programStartTime_multithread;
    std::chrono::high_resolution_clock::time_point programEndTime_multithread;

    std::chrono::duration<double> GlobalPlacementTime_multithread;
    std::chrono::duration<double> DetailPlacementTime_multithread;
    std::chrono::duration<double> LegalizationTime_multithread;
    std::chrono::duration<double> ProgramTime_multithread;

    programStartTime = seconds();
    struct timeval tim;
    gettimeofday(&tim,nullptr);
    //double programStartTime_multithread = tim.tv_sec + (tim.tv_usec/1000000.0);
    programStartTime_multithread = std::chrono::high_resolution_clock::now();
    cout << "\n";
    cout << "==============================================================\n\n";
    ...
    //double GlobalPlacementStartTime_multithread;
    ...
    if( bRunGlobal )
    {
        ...
        //GlobalPlacementStartTime_multithread = tim.tv_sec + (tim.tv_usec/1000000.0);
        GlobalPlacementStartTime_multithread = std::chrono::high_resolution_clock::now();
        ...
        GlobalPlacementEndTime_multithread = std::chrono::high_resolution_clock::now();
        GlobalPlacementTime_multithread = std::chrono::duration_cast< std::chrono::duration<double> >( GlobalPlacementEndTime_multithread - GlobalPlacementStartTime_multithread );
        if( bOutInterPL )
        {
    ...
    //double LegalStartTime_multithread = tim.tv_sec + (tim.tv_usec/1000000.0);
    LegalizationStartTime_multithread = std::chrono::high_resolution_clock::now();
    ...
    if( bRunLegal )	// add "!isLegal" (donnie, 2006-03-06)
    {
        ...
        else
        {
            ...
            //double LegalEndTime_multithread = tim.tv_sec + (tim.tv_usec/1000000.0);
            LegalizationEndTime_multithread = std::chrono::high_resolution_clock::now();
            LegalizationTime_multithread = std::chrono::duration_cast< std::chrono::duration<double> >( LegalizationEndTime_multithread - LegalizationStartTime_multithread );
            //total_legal_time = LegalEndTime_multithread - LegalStartTime_multithread;
   ...
   // double DetailPlacementStartTime_multithread = tim.tv_sec + (tim.tv_usec/1000000.0);
    DetailPlacementStartTime_multithread = std::chrono::high_resolution_clock::now();
    double wl3 = 0;
    ...
    //double DetailPlacementEndTime_multithread = tim.tv_sec + (tim.tv_usec/1000000.0);
    //total_detail_time = DetailPlacementEndTime_multithread - DetailPlacementStartTime_multithread;
    DetailPlacementEndTime_multithread = std::chrono::high_resolution_clock::now();
    DetailPlacementTime_multithread = std::chrono::duration_cast< std::chrono::duration<double> >( DetailPlacementEndTime_multithread - DetailPlacementStartTime_multithread );
    ...
    //double programEndTime_multithread = tim.tv_sec + (tim.tv_usec/1000000.0);
    programEndTime_multithread = std::chrono::high_resolution_clock::now();
    //total_time = programEndTime_multithread - programStartTime_multithread;
    ProgramTime_multithread = std::chrono::duration_cast< std::chrono::duration<double> >( programEndTime_multithread - programStartTime_multithread );



    total_part_time = GlobalPlacementTime_multithread.count();
    total_legal_time = LegalizationTime_multithread.count();
    total_detail_time = DetailPlacementTime_multithread.count();
    total_time = ProgramTime_multithread.count();
    ...
    //    printf("param.count_updatepotential = %d,param.time_updatepotential = %f\n",
    //           param.count_updatepotential,param.time_updatepotential);
    //    printf("param.count_updatepotential_adjust = %d,param.time_updatepotential_adjust = %f\n",
    //           param.count_updatepotential_adjust,param.time_updatepotential_adjust);


    ofstream outfile;
    outfile.open("record.log",ios::app);
    outfile << endl;
    outfile << "Circuit: " << argv[2] << endl;
    outfile << "step   : " << param.step << endl;
    outfile << "GlobalPlacement Time:" << total_part_time      << " (s) \t" << total_part_time/60       << " (min) " << endl;
    outfile << "Legal Time:" << total_legal_time      << " (s) \t" << total_legal_time/60       << " (min) " << endl;
    outfile << "DetailPlacement Time:" << total_detail_time    << " (s) \t" << total_detail_time/60       << " (min) " << endl;
    outfile << "Total  Time:\t" << total_time      << " (s) \t" << total_time/60       << " (min) "
            << "\twire:\t"<< placedb.GetHPWLp2p() << endl;
